...
def parse_args(argv=None):
    ...
    p.add_argument(
        ...
        help="Write a SQL script that DELETEs the BookFiles rows for every "
        "MISMATCH, to disassociate them in Readarr WITHOUT deleting the files "
        "on disk. Review before running it. Covers ALL mismatches, not just "
        "the --limit shown. (env: EMIT_SQL)",
    )
    p.add_argument(
        "--emit-copy",
        metavar="PATH",
        default=os.environ.get("EMIT_COPY"),
        help="Write a shell script with a 'cp -r' command per MISMATCH that "
        "copies the file's containing directory into the manual-import staging "
        "dir (see --copy-dest), so you can re-import it against the correct "
        "book. Covers ALL mismatches regardless of --limit. (env: EMIT_COPY)",
    )
    p.add_argument(
        "--copy-dest",
        default=os.environ.get("COPY_DEST", "/data/media/Download/manual-import/"),
        help="Destination directory for the --emit-copy commands. "
        "(env: COPY_DEST)",
    )
    p.add_argument(
        "--copy-tool",
        choices=("cp", "rsync"),
        default=os.environ.get("COPY_TOOL", "cp"),
        help="Command used by --emit-copy. 'rsync' supports a real progress "
        "bar (combine with --copy-progress). (env: COPY_TOOL)",
    )
    p.add_argument(
        "--copy-progress",
        action="store_true",
        default=env_bool(os.environ.get("COPY_PROGRESS", "")),
        help="Show per-file/transfer progress in --emit-copy: 'cp -rv' prints "
        "each file as it copies; with --copy-tool rsync it adds "
        "'--info=progress2' for a percentage bar. (env: COPY_PROGRESS)",
    )
...
def write_unlink_sql(path, mismatches):
    """Write a reviewable SQL script that unlinks each mismatched BookFile.

    Deleting the BookFiles row removes Readarr's association but leaves the file
    on disk (unlike the API's DELETE /bookfile/{id}, which deletes the file).
    Only rows with a known file_id are emitted.
    """
    lines = [
        "-- Generated by compare_readarr.py --emit-sql",
        "-- Each statement disassociates a mismatched file from its book by",
        "-- removing the BookFiles row. The file on disk is NOT touched.",
        "--",
        "-- BEFORE RUNNING:",
        "--   1. Back up the database:  pg_dump readarr-main > readarr-backup.sql",
        "--   2. Stop Readarr (or restart it afterward) so it reloads state.",
        "--   3. Review each line below and delete any you don't want applied.",
        "--",
        "-- After unlinking, the file shows as ORPHANED. To stop a rescan from",
        "-- re-linking it to the same wrong book, move/rename it to the correct",
        "-- book's folder or fix it via Readarr's Manual Import.",
        "",
        "BEGIN;",
        "",
    ]
    ...
    for m in mismatches:
        ...
        lines.append(f"-- {m['db_path']}")
        lines.append(f"--   book: {m['author']} - {m['book']}  ({reason})")
        lines.append(f'DELETE FROM "BookFiles" WHERE "Id" = {int(m["file_id"])};')
        lines.append("")
        emitted += 1
    lines.append("COMMIT;")
    lines.append("")
...
def write_copy_script(path, mismatches, dest, tool="cp", progress=False):
    """Write a shell script that copies each mismatched book's folder to `dest`.

    The directory copied is the one containing the reported file (its book
    folder). Files are *copied* (originals stay in place), so the result can be
    re-imported via Readarr's Manual Import without disturbing the library.
    Identical source directories are emitted only once.

    `tool` selects 'cp' or 'rsync'. With `progress=True`, cp gains '-v' (prints
    each file) and rsync gains '--info=progress2' (a percentage bar).
    """
    if tool == "rsync":
        copy_cmd = "rsync -a --info=progress2" if progress else "rsync -a"
    else:
        copy_cmd = "cp -rv" if progress else "cp -r"
    lines = [
        "#!/bin/sh",
        "# Generated by compare_readarr.py --emit-copy",
        "# Copies the containing folder of each MISMATCH file into the",
        "# manual-import staging directory, so it can be re-imported against",
        "# the correct book in Readarr. Originals are left untouched.",
        "#",
        "# Review before running. Paths are shell-quoted for spaces/specials.",
        "set -e",
    ...
    seen = set()
    for m in mismatches:
        src = posixpath.dirname(m["path"])
        if not src or src in seen:
            continue
        seen.add(src)
        reason = "; ".join(m["reasons"])
        lines.append(f"# {m['author']} - {m['book']}  ({reason})")
        lines.append(f"{copy_cmd} {shlex.quote(src)} {shlex.quote(dest)}")
        lines.append("")
        emitted += 1
    with open(path, "w", encoding="utf-8") as fh:
        fh.write("\n".join(lines))
...
def main(argv=None):
    ...
    if args.emit_sql:
        emitted = write_unlink_sql(args.emit_sql, mismatches)
        print(f"Unlink SQL for {emitted} mismatch(es) written to: {args.emit_sql}")
        print("Review it, back up the DB, and stop Readarr before applying.")

    if args.emit_copy:
        emitted = write_copy_script(args.emit_copy, mismatches, args.copy_dest,
                                    tool=args.copy_tool,
                                    progress=args.copy_progress)
        print(f"Copy script for {emitted} folder(s) written to: {args.emit_copy}")
        print(f"It copies each mismatched book folder into {args.copy_dest}")
