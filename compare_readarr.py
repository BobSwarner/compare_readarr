...
def parse_args(argv=None):
    ...
    p.add_argument(
        ...
        help="Make --emit-copy use 'cp -rv' so each file is printed as it is "
        "copied (cp has no percentage bar; use rsync/pv for that). "
        "(env: COPY_PROGRESS)",
    )
...
def write_copy_script(path, mismatches, dest, progress=False):
    ...
    Identical source directories are emitted only once. With `progress=True`,
    `cp -rv` is used so each file prints as it is copied.
    """
    cp_flags = "-rv" if progress else "-r"
    lines = [
        "#!/bin/sh",
    ...
    for m in mismatches:
        ...
        lines.append(f"cp {cp_flags} {shlex.quote(src)} {shlex.quote(dest)}")
        lines.append("")
        emitted += 1
    with open(path, "w", encoding="utf-8") as fh:
        fh.write("\n".join(lines))
